...
pae.heatmap <- function(import_afm, afm_complex = "NSP14-NSP10", afm_rank = 1, interface_cluster = FALSE, plldt = 50) {
  ...
  length_A <- import_afm$protein %>%
    dplyr::filter(A_protein == str_split(afm_complex, pattern = "-")[[1]][[1]] & B_protein == str_split(afm_complex, pattern = "-")[[1]][[2]]) %>%
    dplyr::pull(A_length)
  ...
  if(interface_cluster == FALSE) {
    plot <- ComplexHeatmap::Heatmap(pae_matrix, cluster_rows = FALSE, cluster_columns = FALSE,
                                       row_split = c(rep(tolower(protein_A), length_A), rep(tolower(protein_B), length_B)),
                                       column_split = c(rep(tolower(protein_A), length_A), rep(tolower(protein_B), length_B)),
    ...
    barplot <- afm_pae_df %>%
      ...
      ggplot2::scale_fill_gradientn(colors = c("blue", "white", "red"), limit = c(0, 30)) +
      ggplot2::theme_bw() +
    ...
    if(length_A > length_B) {
      plot <- cowplot::plot_grid(
        ...
        cowplot::plot_grid(grid.grabExpr(draw(heatmap_interAB)), NULL,
                           ...
                           ncol = 1, rel_heights = c(nrow(afm_pae_interAB)/nrow(afm_pae_interBA), 0.5, 0.5)),
        ncol = 2, rel_widths = c(1, ncol(afm_pae_interAB)/ncol(afm_pae_interBA)))
    }
    if(length_B > length_A) {
      plot <- cowplot::plot_grid(
        ...
        cowplot::plot_grid(grid.grabExpr(draw(heatmap_interBA)), NULL,
                           cowplot::plot_grid(barplot, NULL, ncol = 2),
                           ncol = 1, rel_heights = c(nrow(afm_pae_interBA)/nrow(afm_pae_interAB), 0.5, 0.5)),
        ncol = 2, rel_widths = c(1, ncol(afm_pae_interBA)/ncol(afm_pae_interAB)))
    }
